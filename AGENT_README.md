...
This document outlines common code cleanup and documentation maintenance tasks that can be performed systematically on
this codebase.
...
1. **Review all module-level docstrings**
   ...
   for file in src/lfp_build/*.py; do
...
6. **Run linter checks**
   ```bash
   uv run ruff check src/lfp_build/
   read_lints src/lfp_build/
   ```
...
```bash
uv run lfp-build sync ruff
```

### 3. Clean Build Artifacts

```bash
uv run lfp-build clean build-artifacts
```
...
- [ ] Run `uv run lfp-build readme update-cmd` to keep CLI help current
...
grep -B 1 "^def " src/lfp_build/*.py | grep -v '"""'
...
uv run lfp-build readme update-cmd --no-write
```
...
- See `src/lfp_build/readme.py` for README automation implementation
